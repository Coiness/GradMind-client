...
describe("workflowExecutor - extractSourceValue", () => {
  ...
  it("应该从对象中提取指定字段", () => {
    ...
    expect(extractSourceValue(sourceResult, "u")).toEqual([
      [1, 2],
      [3, 4],
    ]);
    expect(extractSourceValue(sourceResult, "sigma")).toEqual([5, 6]);
    expect(extractSourceValue(sourceResult, "vt")).toEqual([
      [7, 8],
      [9, 10],
    ]);
  });
  ...
  it("字段不存在时应该返回整个对象", () => {
    ...
    expect(extractSourceValue(sourceResult, "nonexistent")).toEqual({
      data: [1, 2, 3],
    });
  });
