...
function buildChartOption(
  vizData: Record<string, unknown> | null,
): object | null {
  if (!vizData) return null;

  // 如果上游节点已经给出了 visualization 结构，直接用
  const viz = vizData.visualization as
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    ...
    const innerViz = rawObj.visualization as
      | { type: string; data: Record<string, unknown> }
      | undefined;
...
function buildFromVisualization(
  type: string,
  ...
  switch (type) {
        ...
        series: [
          {
            ...
            symbolSize: 5,
            itemStyle: { color: "#722ed1", opacity: 0.75 },
          },
        ],
      };
    }
    case "convergence": {
      ...
      return {
        ...
        xAxis: {
          ...
          axisLabel: { fontSize: 9, interval: Math.floor(history.length / 4) },
        },
        yAxis: {
    ...
    case "training": {
      ...
      return {
        ...
        xAxis: {
          type: "category",
          data: loss.map((_, i) => i + 1),
          axisLabel: { fontSize: 9, interval: Math.floor(loss.length / 4) },
        },
        yAxis,
        series,
      };
    ...
    case "regression": {
      ...
      return {
        title: {
          text: `R²=${((data.rSquared as number) ?? 0).toFixed(3)}`,
          textStyle: { fontSize: 11 },
          left: "center",
          top: 2,
        },
        ...
        series: [
          {
            type: "scatter",
            data: xVals.map((x, i) => [x, yData[i]]),
            symbolSize: 5,
            itemStyle: { color: "#5470c6", opacity: 0.7 },
          },
          {
            type: "line",
            data: sorted.map(({ x, i }) => [x, predictions[i]]),
            showSymbol: false,
            lineStyle: { color: "#722ed1", width: 2 },
          },
        ],
...
export const OscilloscopeNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  useEffect(() => {
    ...
    // 多图层图像渲染 (K-Means)
    if (
      (chartOption as any)._customRender === "image-collection" &&
      canvasRef.current
    ) {
      ...
      drawMatrix(
        mainImage,
        padding + curCol * (scaledWidth + padding),
        padding + curRow * (scaledHeight + padding),
        "合成效果",
      );
      curCol++;
  ...
  return (
    <div
      className={`${styles.oscilloscopeNode} ${selected ? styles.selected : ""}`}
    >
      {/* Input Handle */}
      <Handle
        type="target"
      ...
      <Card size="small" className={styles.card}>
        ...
        <div className={styles.chartArea}>
          {chartOption &&
          ((chartOption as any)._customRender === "image" ||
            (chartOption as any)._customRender === "image-collection") ? (
            <div
              style={{
                width: "100%",
